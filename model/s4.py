...
def cauchy_conj_torch(v, z, w):
    """Native PyTorch equivalent of the conjugate-symmetric PyKeOps kernel."""
    z_expanded = z.unsqueeze(-2)
    w_expanded = w.unsqueeze(-1)
    v_expanded = v.unsqueeze(-1)
    numerator = z_expanded * v_expanded.real - (v_expanded * w_expanded.conj()).real
    denominator = (z_expanded - w_expanded) * (z_expanded - w_expanded.conj())
    return 2 * torch.sum(numerator / denominator, dim=-2)
...
try:
    ...
    def log_vandermonde(v, x, L):
        ...
        vandermonde_mult = Genred(
            expr, ["v = Vj(2)", "x = Vj(2)", "l = Vi(2)"], reduction_op="Sum", axis=1
        )
...
class SSKernelNPLR(OptimModule):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        elif has_pykeops and os.environ.get(
            "S4ITNET_DISABLE_KEOPS", "0"
    ...
    def default_state(self, *batch_shape):
        ...
        if step_mode != "linear":
            ...
            self.input_contraction = contract_expression(
                "h n, ... h -> ... h n", (H, N), batch_shape + (H,)
            )
        self.output_contraction = contract_expression(
            "c h n, ... h n -> ... c h", (C.shape[0], H, N), batch_shape + (H, N)
        )
...
class S4Model(nn.Module):
    def __init__(self, d_input, d_model=128, n_layers=4, dropout=0.1, l_max=1024):
        super().__init__()
    ...
    def forward(self, x, lengths=None):
        """
        x: shape is (B, L, d_input)    (64, 1000, 22)
        """
